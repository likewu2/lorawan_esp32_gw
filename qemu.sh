...
$ESP_QEMU_PATH/qemu-system-xtensa -nographic -M esp32 -no-reboot -nic user,model=open_eth,id=lo0,hostfwd=tcp::7888-:80 -drive file=build/esp32-main-qemu.bin,if=mtd,format=raw
...
$ESP_QEMU_PATH/qemu-system-xtensa -nographic -M esp32 -m 4 -no-reboot -nic user,model=open_eth,id=lo0,hostfwd=tcp::8000-:80 -drive file=build/qemu-esp32-wifitest.bin,if=mtd,format=raw
